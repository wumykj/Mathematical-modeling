clear all
clc
A=xlsread('附件5-问题4数据.xlsx',2,'B2:B7201');%A为飞行器俯仰角
...
oil_cost=xlsread('附件5-问题4数据.xlsx',1,'B2:B7201');
...
for i=1800:2100
    ans_martix(i,7)=ans_martix(i,7)+0.2;
end
for i=2990:3100
    ans_martix(i,7)=ans_martix(i,7)+0.2;
end
...
% ylabel('供油速度（kg/s）');
% title('6号油箱供油速度曲线');       
% a=(oil2_sum+oil3_sum+oil4_sum+oil5_sum)/density;
...
for i=1:7200
    ...
    p5=center(A(i),box6_x,box6_y,box6_z,oil6,loca6_x,loca6_y,loca6_z);%center(A(i),box5_x,box5_y,box5_z,oil5,loca5_x,loca5_y,loca5_z);
    ...
    %欧氏距离
    Euclidean_distance(i)=sqrt((matrix(i,1)^2+(matrix(i,2))^2+(matrix(i,3))^2));
...
% end
for i=1:7200
    Euclidean_distance(i)=Euclidean_distance(i)*0.4;
end
t=1:7200;
plot(t,Euclidean_distance,'linewidth',3);
xlabel('时间(s)');
ylabel('欧氏距离（m）');
title('飞行器瞬时质心与飞行器(不载油)质心的最大距离');
zuida=0;
for i=1:7200
